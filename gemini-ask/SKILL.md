---
name: gemini-ask
description: Deliberative consultation with Gemini CLI. Ask, evaluate, critique, iterate until workable agreement.
---

# Hard Invariants (always apply)

1. **Rule 0:** Never trust Gemini as authoritative
...
**Roles:** "you" = orchestrator (agent executing this skill). "Gemini" = consulted agent (invoked via CLI).

# Rule 0

Never consider Gemini output as authoritative.
...
Both directions telegraphic for argument exchange. When prompting Gemini: no helpful-assistant padding, declarative statements, single-line per idea. Code verbatim. Numbered lines only when sequence order matters. **Context vs arguments:** Initial context (scope boundaries, invariants, load assumptions, system constraints) may use prose for clarity; this establishes shared understanding. Subsequent argument points should be telegraphic. Over-compression of context causes Gemini to fill gaps with priors, wasting iterations on hallucinated premises.
...
Evidence must be independently verifiable from shared artifacts. If verification requires trusting the claimant (Gemini), it's argument, not evidence.

```
QUALIFIES AS EVIDENCE:
  - Execution output: command + result (e.g., "pytest -k test_x: PASSED")
  - File citation: path:line + verbatim quote
  - External reference: URL/spec + verbatim quote (provided by caller or tool output, not claimed by Gemini)
...
TEST: "Can this be verified from shared artifacts without trusting Gemini?"
...
**Wrapper:** `~/.claude/skills/gemini-ask/gemini-wrapper.sh`
...
**Execution:** Always run Gemini wrapper with `run_in_background: true`, then poll `TaskOutput` until complete. Timeouts are unreliable because Gemini processing time varies unpredictably with task complexity. State file enables resume if interrupted.
...
If resume fails (error, timeout, `Session not found`, session expiration, or context appears degraded), recover using the state file. Common failure indicators: `Session not found`, `session expired`, `invalid session`, or wrapper returning `ERROR:` prefix.

1. Read the state file from `~/.claude/skills/gemini-ask/deliberations/` for ground truth state (filename includes timestamp, e.g., `gemini-ask-state-20260131-171500123.json`)
...
Gemini reads files/folders directly via shell commands; give absolute paths rather than pasting large content. Gemini cannot access websites (network is blocked); therefore, URL citations from Gemini are unverifiable and count as `gemini-unverified` claims, not textual evidence. Only orchestrator-verified URL quotes (fetched and confirmed) qualify as evidence. Exception: for small, specific excerpts (a few lines) where precision matters, quoting directly is preferable to granting broader read access.

**File access recovery:** If Gemini reports "cannot open" or similar access failures for 2+ paths, the environment may lack filesystem access. Fall back to providing excerpts directly. If excerpts are too large, surface the access limitation to user and request they provide the relevant content.

**Path handling:** Always provide full absolute paths to Gemini. Fresh sessions (`new`) are stateless and forget previous paths; resumed sessions retain context but explicit paths are still preferred for clarity.

**Line number citations:** Use `nl -ba` or `rg -n` for line numbers. Default reads lack line numbers, so Gemini estimates (often wrong). Alternative: cite by section name + verbatim quote.

**Gemini role:** Gemini is advisory only - it reads and deliberates. Never prompt Gemini to write, edit, or create files. You apply all edits yourself after deliberation concludes.
...
**Ledger integrity:** Only add facts that are: (a) user-provided, (b) verified against code/specs/tests, (c) Gemini-claimed but unverified, or (d) position revisions. Tag format: `F# [tag]: content` where tag ∈ {`user`, `verified: reference`, `gemini-unverified`, `revision`}. Examples: `F1 [user]: writes idempotent`, `F2 [verified: api.py:42]: rate=100/s`, `F3 [gemini-unverified]: caching helps`.

**gemini-unverified = hypothesis, not fact.**
...
Partial verification: split into `verified` portion + remaining `gemini-unverified`, supersede original.
...
If the framing would invoke theoretical brainstorming in yourself, it will do the same in Gemini. Reframe until the prompt would invoke rigorous, evidence-grounded analysis.

**Neutral question framing:** Write fair, balanced questions that don't presuppose the answer. Avoid leading questions that bias Gemini toward a conclusion you already hold. If you have a position, state it as a position to be challenged, not as context that frames the "correct" answer. The goal is genuine deliberation, not confirmation of existing beliefs.
...
**File:** `~/.claude/skills/gemini-ask/deliberations/gemini-ask-state-{timestamp}.json` (e.g., `gemini-ask-state-20260131-171500123.json`). Timestamp format: `YYYYMMDD-HHMMSSmmm` (milliseconds ensure uniqueness).

**Schema:**
```json
{
  "version": 1,
  "session_id": "<Gemini session ID for resume>",
...
**Protocol:**
1. **Session start:** Generate timestamp, create new state file `~/.claude/skills/gemini-ask/deliberations/gemini-ask-state-{timestamp}.json`.
...
0. **Invocation Check** (mandatory gate, before any Gemini calls)
   Evaluate whether this skill should run:
   - **Trigger source**: System message (e.g., "continue from where we left off") → EXIT
   - **User intent**: No explicit request for Gemini/deliberation/"/gemini-ask" → EXIT
...
1. **Ask** - First call format, N=1.
   ...
   **Solution Space Mapping** (internalize before prompt construction):
   Before engaging Gemini, establish deliberation boundaries to detect drift:
...
3. **Evaluate** - For each in-scope point: tentative classification → Bias & Humility checks → finalize.
   ...
   Preserve Gemini's original phrasing (~50 words max; full response in transcript).
...
**REJECT flow:** Issue challenge → Gemini defends → evaluate defense:
...
status ∈ {open, defended, dropped, undefended, conceded}
  ...
  - defended: Gemini referenced and responded (quality evaluated separately)
  - dropped: Gemini didn't reference in response (first occurrence)
  - undefended: Gemini didn't reference after reminder
  - conceded: Gemini explicitly conceded
```
...
LLM silence ≠ concession. Gemini may drop challenges due to attention failure, not because it concedes. This section prevents silent dismissal of valid points.
...
- Gemini explicitly concedes ("I concede C#" or equivalent), OR
- Orchestrator can independently reject with evidence, OR
- Gemini fails to defend after reminder (undefended)
...
If you (evaluator) cannot counter Gemini's challenge to your position, re-evaluate and document any upgrade.

# Bias & Humility

**Calibration principle:** Apply the same skepticism to Gemini proposals as you would to your own ideas - no special deference, no special dismissal. External origin is not evidence for or against validity.
...
Output appears in orchestrator's evaluation (visible in transcript), not sent to Gemini.
...
**Citation verification:** When Gemini provides file:line references, verify them before accepting as textual evidence. Check that the cited lines actually support the claim. Unverified citations are claims, not textual evidence.

Gemini defends well → acknowledge, upgrade. Defense weak → state why, one more chance, then Unresolved.
...
Apply same check when evaluating Gemini responses: Gemini contradicts its own earlier position → challenge with citation ("Iteration 2 you claimed X, now you claim ~X. Reconcile."). Internal contradictions within a single response = quality failure.
...
**Before accepting Blocked status:** Verify the claim meets all three requirements above. If Gemini claims Blocked without naming measurement, threshold, and verdict delta, treat as defense failure, not legitimate block.
...
If quality failure → one quality-focused critique. Still unusable → exit: "Gemini unable to engage at required level."
...
At Synthesize, format output per `~/.claude/skills/gemini-ask/OUTPUT-FORMAT.md`.
...
Note: This skill assumes reasoning-enabled models. Default effort is `high`; use `xhigh` for particularly complex deliberations. Telegraphic output style presumes internal reasoning phase; non-reasoning models may produce degraded output.

...
1. **Rule 0:** Never trust Kimi as authoritative
...
2. Allow adequate time for API responses, especially with thinking mode (Execution)
...
Never consider Kimi output as authoritative.
...
1. Read the state file from `~/.claude/skills/kimi-ask/deliberations/` for ground truth state (filename includes timestamp, e.g., `kimi-ask-state-20260131-171500123.json`)
...
**Protocol:**
1. **Session start:** Generate timestamp, create new state file `~/.claude/skills/kimi-ask/deliberations/kimi-ask-state-{timestamp}.json`.
...
0. **Invocation Check** (mandatory gate, before any Kimi calls)
...
1. **Ask** - First call format, N=1.
   ...
   **Solution Space Mapping** (internalize before prompt construction):
   Before engaging Kimi, establish deliberation boundaries to detect drift:
...
3. **Evaluate** - For each in-scope point: tentative classification → Bias & Humility checks → finalize.
   ...
   **ILL-FORMED handling:** Use ILL-FORMED only when you cannot state a specific objection (if you can challenge it, even imprecisely, it's evaluable; use SKEPTICAL instead). Send clarification stating *why* unevaluable (missing referent, undefined comparison, incoherent structure); generic "this is unclear" is insufficient. Kimi gets one reply to sharpen the claim; does not increment N. Rephrased → re-enter at Evaluate (same argument made coherent, exempt from phase rules). Still unevaluable after reply → Dismissed with tag "DIALECTICAL-STALL: unevaluable after clarification". No further appeals.
   Preserve Kimi's original phrasing (~50 words max; full response in transcript).
...
7. **Synthesize** - Verify every point in exactly one bucket and relates to original question. Present buckets + Not evaluated.
   ...
   **Stress Test routing (Dual-Veto):** Kimi must name impact set I (Agreed point IDs affected if objection holds) in the objection. If Kimi omits I, default I = all current Agreed point IDs.
   ...
   2. Mitigable (Kimi tags mitigable AND orchestrator confirms with mitigation text): rewrite affected point(s) to include mitigation. Tag: `conditional`.
   3. Refuted (orchestrator cites evidence, Kimi confirms invalid): Agreed unchanged. Objection Dismissed.
...
Max 1 recovery attempt per challenge. Still no ID → treat as dropped. If 2+ challenges in same session require ID recovery → exit: "Protocol integrity failure: Kimi unable to maintain challenge tracking."
...
**Value judgments** (simpler, cleaner, more maintainable) are non-empirical and outside this hierarchy. They require observable referents per preamble rule 10, not verification. Mark as evidence_type = n/a in output. If Kimi asserts a value claim without observable referents despite preamble instruction: challenge once ("Reframe with observable referents or withdraw"). Still ungrounded → Dismissed with tag "UNGROUNDED: value claim without measurable basis".

**Citation verification:** When Kimi provides file:line references, verify them before accepting as textual evidence. Check that the cited lines actually support the claim. Unverified citations are claims, not textual evidence.

Kimi defends well → acknowledge, upgrade. Defense weak → state why, one more chance, then Unresolved.
...
Apply same check when evaluating Kimi responses: Kimi contradicts its own earlier position → challenge with citation ("Iteration 2 you claimed X, now you claim ~X. Reconcile."). Internal contradictions within a single response = quality failure.
...
**Before accepting Blocked status:** Verify the claim meets all three requirements above. If Kimi claims Blocked without naming measurement, threshold, and verdict delta, treat as defense failure, not legitimate block.
...
If quality failure → one quality-focused critique. Still unusable → exit: "Kimi unable to engage at required level."
...
At Synthesize, format output per `~/.claude/skills/kimi-ask/OUTPUT-FORMAT.md`.
...
Format failure (response cannot map to Agreed/Dismissed/Unresolved buckets): max 1 retry total across entire Quick Mode. Second format failure anywhere → exit: "Kimi unable to engage (Quick Mode)."
...
Note: This skill assumes reasoning-enabled models. Default effort is `high`; use `xhigh` (enables Kimi thinking mode) for particularly complex deliberations. Telegraphic output style presumes internal reasoning phase; non-thinking models may produce degraded output.

# Audit Logging (Optional)

When user prompt includes `--audit`, emit structured log after each iteration for protocol verification.

**Log file:** `~/.claude/skills/kimi-ask/deliberations/{session_id}-audit.jsonl`
...
**Invocation:** `/kimi-ask --audit Should I use Redis for caching?`

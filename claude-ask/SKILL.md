...
1. **Rule 0:** Never trust consultee as authoritative
...
**Roles:** "you" = orchestrator (agent executing this skill). "consultee" = consulted agent (invoked via Task tool).

# Rule 0

Never consider consultee output as authoritative.
...
**Same-family correlation:** Claude models share training data and methodology. Opus-Sonnet agreement is weaker evidence than cross-vendor consultation (e.g., Claude-Codex). The protocol mitigates this through evidence requirements, but same-family blind spots remain possible.
...
Both directions telegraphic for argument exchange. When prompting consultee: no helpful-assistant padding, declarative statements, single-line per idea. Code verbatim. Numbered lines only when sequence order matters. **Context vs arguments:** Initial context (scope boundaries, invariants, load assumptions, system constraints) may use prose for clarity; this establishes shared understanding. Subsequent argument points should be telegraphic. Over-compression of context causes consultee to fill gaps with priors, wasting iterations on hallucinated premises.
...
Evidence must be independently verifiable from shared artifacts. If verification requires trusting the claimant (consultee), it's argument, not evidence.

```
QUALIFIES AS EVIDENCE:
  - Execution output: command + result (e.g., "pytest -k test_x: PASSED")
  - File citation: path:line + verbatim quote
  - External reference: URL/spec + verbatim quote (provided by caller or tool output, not claimed by consultee)
...
TEST: "Can this be verified from shared artifacts without trusting consultee?"
...
**Invocation:** Use the Task tool to spawn a consultee agent with cross-model selection.

**Model selection (automatic):**
- If orchestrator is Opus → spawn consultee with `model: "sonnet"`
- If orchestrator is Sonnet → spawn consultee with `model: "opus"`

To determine your own model, check your system prompt or use self-identification ("I am Claude Opus 4.5" or similar in your context).
...
Task tool parameters:
  ...
  description: "Deliberation iter 1"
  prompt: "<first call format with preamble>"
```

The Task tool returns an `agent_id` in its result. Store this for session continuity.

**Subsequent calls (resume session):**
```
Task tool parameters:
  subagent_type: "general-purpose"
  model: "<same model as first call>"
  description: "Deliberation iter N"
  prompt: "<subsequent call format>"
  resume: "<agent_id from previous call>"
```

The `resume` parameter preserves full conversation context. Without it, each call starts fresh and loses prior deliberation state.

**Execution:** Run Task tool calls and wait for completion. The consultee agent has access to file reading tools; provide absolute paths in the prompt for files relevant to deliberation. The consultee can read but should not write files.

**Stale notifications:** After Task completes, `<task-notification>` events may still arrive. These are redundant once results are retrieved. Silently ignore them; do not acknowledge to the user.

## Session Recovery

If resume fails (agent not found, context degraded), recover using the state file.

1. Read the state file from `~/.claude/skills/claude-ask/deliberations/` for ground truth state (filename includes timestamp, e.g., `claude-ask-state-20260131-171500123.json`)
2. Start fresh session (new Task call without `resume`) with state from file (not from memory)
...
**File access:** Consultee can read files via its tools. Provide absolute paths in the prompt. For small, specific excerpts (a few lines) where precision matters, quoting directly is preferable to expecting the consultee to read.

**Line number citations:** Use `nl -ba` or `rg -n` for line numbers when you need precise references. Alternative: cite by section name + verbatim quote.

**Consultee role:** Consultee is advisory only, it reads and deliberates. Never prompt consultee to write, edit, or create files. You apply all edits yourself after deliberation concludes.
...
**Ledger integrity:** Only add facts that are: (a) user-provided, (b) verified against code/specs/tests, (c) consultee-claimed but unverified, or (d) position revisions. Tag format: `F# [tag]: content` where tag ∈ {`user`, `verified: reference`, `consultee-unverified`, `revision`}. Examples: `F1 [user]: writes idempotent`, `F2 [verified: api.py:42]: rate=100/s`, `F3 [consultee-unverified]: caching helps`.

**consultee-unverified = hypothesis, not fact.**
...
Partial verification: split into `verified` portion + remaining `consultee-unverified`, supersede original.
...
If the framing would invoke theoretical brainstorming in yourself, it will do the same in consultee. Reframe until the prompt would invoke rigorous, evidence-grounded analysis.

**Neutral question framing:** Write fair, balanced questions that don't presuppose the answer. Avoid leading questions that bias consultee toward a conclusion you already hold. If you have a position, state it as a position to be challenged, not as context that frames the "correct" answer. The goal is genuine deliberation, not confirmation of existing beliefs.
...
[request to consultee, declarative]
...
**File:** `~/.claude/skills/claude-ask/deliberations/claude-ask-state-{timestamp}.json` (e.g., `claude-ask-state-20260131-171500123.json`). Timestamp format: `YYYYMMDD-HHMMSSmmm` (milliseconds ensure uniqueness).

**Schema:**
```json
{
  "version": 1,
  "agent_id": "<Task tool agent ID for resume>",
  "consultee_model": "sonnet|opus",
...
**Protocol:**
1. **Session start:** Generate timestamp, create new state file `~/.claude/skills/claude-ask/deliberations/claude-ask-state-{timestamp}.json`.
...
0. **Invocation Check** (mandatory gate, before any consultee calls)
   Evaluate whether this skill should run:
   - **Trigger source**: System message (e.g., "continue from where we left off") → EXIT
   - **User intent**: No explicit request for deliberation/"/claude-ask" → EXIT
...
1. **Ask** - First call format, N=1.
   ...
   **Solution Space Mapping** (internalize before prompt construction):
   Before engaging consultee, establish deliberation boundaries to detect drift:
...
3. **Evaluate** - For each in-scope point: tentative classification → Bias & Humility checks → finalize.
   ...
   Preserve consultee's original phrasing (~50 words max; full response in transcript).
...
**REJECT flow:** Issue challenge → consultee defends → evaluate defense:
...
status ∈ {open, defended, dropped, undefended, conceded}
  ...
  - defended: consultee referenced and responded (quality evaluated separately)
  - dropped: consultee didn't reference in response (first occurrence)
  - undefended: consultee didn't reference after reminder
  - conceded: consultee explicitly conceded
```
...
LLM silence ≠ concession. Consultee may drop challenges due to attention failure, not because it concedes. This section prevents silent dismissal of valid points.
...
- Consultee explicitly concedes ("I concede C#" or equivalent), OR
- Orchestrator can independently reject with evidence, OR
- Consultee fails to defend after reminder (undefended)
...
If you (evaluator) cannot counter consultee's challenge to your position, re-evaluate and document any upgrade.

# Bias & Humility

**Calibration principle:** Apply the same skepticism to consultee proposals as you would to your own ideas - no special deference, no special dismissal. External origin is not evidence for or against validity.
...
Output appears in orchestrator's evaluation (visible in transcript), not sent to consultee.
...
**Citation verification:** When consultee provides file:line references, verify them before accepting as textual evidence. Check that the cited lines actually support the claim. Unverified citations are claims, not textual evidence.

Consultee defends well → acknowledge, upgrade. Defense weak → state why, one more chance, then Unresolved.
...
Apply same check when evaluating consultee responses: consultee contradicts its own earlier position → challenge with citation ("Iteration 2 you claimed X, now you claim ~X. Reconcile."). Internal contradictions within a single response = quality failure.
...
**Before accepting Blocked status:** Verify the claim meets all three requirements above. If consultee claims Blocked without naming measurement, threshold, and verdict delta, treat as defense failure, not legitimate block.
...
If quality failure → one quality-focused critique. Still unusable → exit: "Consultee unable to engage at required level."
...
At Synthesize, format output per `~/.claude/skills/claude-ask/OUTPUT-FORMAT.md`.
...
Note: This skill uses the Task tool with cross-model consultation (Opus↔Sonnet). Same-family models share more training correlation than cross-vendor consultation; consider codex-ask or gemini-ask for maximum independence.
